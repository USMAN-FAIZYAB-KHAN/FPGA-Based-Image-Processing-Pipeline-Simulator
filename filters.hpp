...
namespace hardware {
    ...
    struct BaseFilter {
        ...
        GrayPixel** window;
        GrayPixel* col; 
        uint32_t kernel_size, kernel_radius;
    ...
    struct BlurFilter : public BaseFilter {
        ...
        bool process(GrayPixel pixelIn, GrayPixel& pixelOut) {
            ...
            #ifdef DEBUG
            if (x_cnt < 5 && y_cnt < 1) {
                std::cout << "[DEBUG BLUR] Window @(" << x_cnt << "," << y_cnt << "):\n";
                for(uint32_t i=0; i<3; ++i) {
                    std::cout << "  [ ";
                    for(uint32_t j=0; j<3; ++j) std::cout << std::setw(3) << (int)window[i][j].value << " ";
                    std::cout << "]\n";
                }
            }
            #endif
            ...
            #ifdef FIXED_POINT
                uint32_t avg = (sum * 7281) >> 16;
                pixelOut = GrayPixel(static_cast<uint8_t>(avg));
            #else
                pixelOut = GrayPixel(static_cast<uint8_t>(sum / 9));
            #endif
    ...
    struct SharpenFilter : public BaseFilter {
        ...
        bool process(GrayPixel pixelIn, GrayPixel& pixelOut) {
            ...
            int32_t result = 0;
            const int weights[3][3] = {{0, -1, 0}, {-1, 5, -1}, {0, -1, 0}};
            ...
            #ifdef DEBUG
            if (x_cnt < 5 && y_cnt < 1) {
                std::cout << "[DEBUG SHARP] Resulting Pixel: " << (int)pixelOut.value << std::endl;
            }
            #endif
    ...
    struct SobelFilter : public BaseFilter {
        ...
        bool process(GrayPixel pixelIn, GrayPixel& pixelOut) {
            ...
            #ifdef DEBUG
            if (x_cnt < 5 && y_cnt < 1) {
                std::cout << "[DEBUG SOBEL] Mag: " << mag << " | Gx: " << gx << " Gy: " << gy << std::endl;
            }
            #endif
